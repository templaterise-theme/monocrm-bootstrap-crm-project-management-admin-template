...
class AdminDashboard {
  ...
  initSidebarCollapse() {
    ...
    if (this.elements.sidebar) {
      this.elements.sidebar.addEventListener("mouseenter", this.handleSidebarHover.bind(this));
      this.elements.sidebar.addEventListener("mouseleave", this.handleSidebarLeave.bind(this));
    }
  }

  collapseSidebar() {
    const classActions = [
      { element: this.elements.sidebar, add: ["sidebar-collapsed"], remove: [] },
      { element: this.elements.icon, add: ["icon-rotated", "fa-arrow-right"], remove: ["fa-bars"] },
      { element: this.elements.collapseMainContent, add: ["collapse-main-collapsed"], remove: ["collapse-main-expanded"] },
      { element: this.elements.header, add: ["header-collapsed"], remove: ["header-expanded"] }
    ];
  ...
  expandSidebar() {
    const classActions = [
      { element: this.elements.sidebar, remove: ["sidebar-collapsed"], add: [] },
      { element: this.elements.icon, remove: ["icon-rotated", "fa-arrow-right"], add: ["fa-bars"] },
      { element: this.elements.collapseMainContent, remove: ["collapse-main-collapsed"], add: ["collapse-main-expanded"] },
      { element: this.elements.header, remove: ["header-collapsed"], add: ["header-expanded"] }
    ];
  ...
  handleSidebarLeave() {
    if (!this.elements.sidebar.classList.contains("sidebar-collapsed") && this.state.isCollapsedManually) {
      this.collapseSidebar();
    }
  }
  ...
  resetSidebarToDefault() {
    const classActions = [
      { element: this.elements.collapseMainContent, remove: ["collapse-main-expanded", "collapse-main-collapsed"] },
      { element: this.elements.header, remove: ["header-expanded", "header-collapsed"] },
      { element: this.elements.sidebar, remove: ["sidebar-collapsed"] },
      { element: this.elements.icon, add: ["fa-bars"], remove: ["fa-arrow-right"] }
    ];
  ...
  handleRowCheckboxChange() {
    const allChecked = Array.from(this.elements.rowCheckboxes).every(checkbox => checkbox.checked);
  ...
  initVariants() {
    ...
    this.elements.addVariantButton?.addEventListener("click", this.addVariant.bind(this));
    this.elements.variantsTable.addEventListener("click", this.handleVariantTableClick.bind(this));
    this.elements.variantsTable.addEventListener("input", this.handleVariantInputChange.bind(this));
    this.elements.variantsTable.addEventListener("change", this.handleVariantFileUpload.bind(this));
  }
    ...
    return `
      ...
      <td>
        <select class="form-select" data-index="${index}" data-field="size">
          ${this.createSelectOptions(['Choose Size', 'S', 'M', 'L'], variant.size)}
        </select>
      </td>
      <td>
        <select class="form-select" data-index="${index}" data-field="color">
          ${this.createSelectOptions(['Choose Color', 'Red', 'Blue', 'Green'], variant.color)}
        </select>
      </td>
      <td width="200px">
  ...
  handleVariantTableClick(e) {
    ...
    if (target.matches("[data-action='increase']")) {
      ...
      this.renderVariants();
    } else if (target.matches("[data-action='decrease']") && this.state.variants[index].quantity > 0) {
      this.state.variants[index].quantity--;
      this.renderVariants();
  ...
  initPrintInvoice() {
    this.elements.printInvoiceButton?.addEventListener("click", this.handlePrintInvoice.bind(this));
  }
  ...
  initCKEditor() {
    if (this.elements.ckeditor) {
      ClassicEditor.create(this.elements.ckeditor)
        .catch(error => console.error("Error initializing CKEditor 5", error));
    }
  }
  ...
  handleProfileUpload(event) {
    ...
    const reader = new FileReader();
    reader.onloadend = () => this.processProfileImageUpload(holder, wrapper, triggerInput, reader.result, currentImg);
    reader.readAsDataURL(file);
  }

  processProfileImageUpload(holder, wrapper, triggerInput, newImageSrc, currentImg) {
    ...
    setTimeout(() => {
      ...
      if (isSuccess) {
        this.showAlert(wrapper, '<i class="fa fa-check-circle text-success"></i> Profile image updated successfully', "snackbar");
      } else {
        holder.querySelector(".pic").src = currentImg;
        this.showAlert(wrapper, '<i class="fa fa-times-circle text-danger"></i> There was an error while uploading! Please try again later.', "snackbar");
      }
      triggerInput.value = "";
    }, 1500);
  }
  ...
  initModals() {
    this.elements.modals.forEach(modal => {
      const dismissButtons = modal.querySelectorAll('[data-modal-dismiss="modal"]');
      const targetButtons = document.querySelectorAll(`[data-modal-target="#${modal.id}"]`);
  ...
  initEmailSidebar() {
    ...
    hamburgerMenu.addEventListener("click", () => this.openEmailSidebar(sidebarEmail, sidebarOverlay));
    sidebarOverlay.addEventListener("click", () => this.closeEmailSidebar(sidebarEmail, sidebarOverlay));

    // Close on outside click
    document.addEventListener("click", (e) => {
      if (sidebarEmail.classList.contains("show") && 
  ...
  setupEmailEventListeners(elements) {
    const emailActions = [
      { element: elements.replyBtn, handler: () => this.handleEmailAction(elements, "Re: ") },
      { element: elements.replyAllBtn, handler: () => this.handleEmailAction(elements, "Re: ") },
      { element: elements.forwardBtn, handler: () => this.handleEmailAction(elements, "Fwd: ") },
      { element: elements.cancelReplyBtn, handler: () => this.handleCancelReply(elements) },
      { element: elements.sendReplyBtn, handler: () => this.handleSendReply(elements) },
      { element: elements.sendBtn, handler: () => this.handleSendEmail(elements) }
    ];
  ...
  handleSendEmail(elements) {
    const { toField, subjectField, messageBody, composeModal, composeForm } = elements;
  ...
  initCalendar() {
    ...
    this.calendar = new FullCalendar.Calendar(this.elements.calendarEl, {
      ...
      editable: true,
      dayMaxEvents: true,
      dateClick: (info) => this.openEventModal({
        id: "",
        title: "",
  ...
  initCalendarEventListeners() {
    ...
    deleteEventBtn?.addEventListener("click", this.handleDeleteEvent.bind(this));
  ...
  openEventModal(eventData) {
    ...
    elements.eventStart.value = eventData.start ? this.formatDateTime(eventData.start) : "";
    elements.eventEnd.value = eventData.end ? this.formatDateTime(eventData.end) : "";
    elements.eventDescription.value = eventData.description;
    
    // Update modal title and button text
    elements.eventModalLabel.textContent = eventData.isNew ? "Add Event" : "Edit Event";
    elements.saveEventBtn.textContent = eventData.isNew ? "Add Event" : "Update Event";
